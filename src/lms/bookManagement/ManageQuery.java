...
public class ManageQuery {
	...
	public static ResultSet showAllBooks(Connection conn, int table_field, int tab_field) {
		StringBuffer sb = new StringBuffer();
		sb.append("select b.book_id, g.genre, b.book_name,b.publisher, p.publisher_id, b.writer, b.book_same_amount, d.book_regist_date, s.book_state");
		sb.append(" from lib_book b, lib_genre g, lib_publisher p, lib_book_detail d, lib_book_state s");
		sb.append(" where( b.genre=g.genre_id"); // 분류
		sb.append(" and b.publisher=p.publisher_id"); // 출판사
		sb.append(" and b.book_id=d.book_id"); // 도서 등록일
	...
	public static void registBook(Connection conn, String[] data, long barcode) {
		int newBookId = getLastBookNumber(conn)+1;
		System.out.println(newBookId);
		StringBuffer sb = new StringBuffer();
		sb.append("insert into lib_book(book_id, genre, book_name, publisher, writer)");
		sb.append(" values("+newBookId+", "+data[0]+", '"+data[1]+"', "+data[2]+", '"+data[3]+"')");
		System.out.println(sb.toString());
		try {
		...
		registBookDetail(conn, newBookId, barcode);
	}
	...
	private static void registBookDetail(Connection conn, int book_id, long barcode) {
		StringBuffer sb = new StringBuffer();
		sb.append("insert into lib_book_detail(barcode, book_id, book_state, book_regist_date, rental_state)");
		sb.append(" values("+barcode+", "+book_id+", 1, default, 1)");
		try {
			pstmt = conn.prepareStatement(sb.toString());
			pstmt.executeUpdate();
		} catch (SQLException e) {
	...
	public static void requestEdit(Connection conn, String[] str, int request_id, long barcode) {
		int result = 0;
		int result2 = 0;
		int book_id = getLastBookNumber(conn)+1;
		
		StringBuffer sb = new StringBuffer();
		StringBuffer sb3 = new StringBuffer();
		PreparedStatement pstmt2 = null;
		sb.append("insert into lib_book(book_id,genre,book_name,publisher,writer)");
		sb.append("values("+book_id+", "+ Integer.parseInt(str[3]) + ",'" + str[0] + "', "+ getPublisherId(conn, str[1]) + ", '" + str[2] + "')");

		sb3.append("insert into lib_book_detail(barcode,book_id,book_state,book_regist_date,rental_state)");
		sb3.append("values("+barcode+", "+book_id+ ", 1, default, 1)");
