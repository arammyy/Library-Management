...
public class BookRequest extends JPanel {
	...
	public void edit(long barcode) {
		String[] str = new String[5];

		str[0] = t_name.getText();
		str[1] = t_publisher.getText();
		str[2] = t_writer.getText();
		str[3] = Integer.toString(ch_category.getSelectedIndex());
		str[4] = Integer.toString(ch_condition.getSelectedIndex());
		if(ManageQuery.isExistPublisher(conn, str[1])) {
			ManageQuery.requestEdit(conn, str, request_id, barcode);
		}else{
			ManageQuery.addNewPublisher(conn, str[1]);
			ManageQuery.requestEdit(conn, str, request_id, barcode);
		}
	...
	public void delete() {
		...
		String sql = "delete from lib_book_request where req_id=" + request_id;
